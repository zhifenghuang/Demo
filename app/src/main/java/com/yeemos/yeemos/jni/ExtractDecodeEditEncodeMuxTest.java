...
public class ExtractDecodeEditEncodeMuxTest {
    ...
    private static final long MAX_VIDEO_TIME = 8 * 1000000; //读取视频最大长度10s
...
//    private int mSourceResId;
    ...
    public void init(String sourcePath, String savePath, int videoType, int userType, Handler handler) {
        ...
        if (width == 0 || height == 0) {
            ...
            float scaleX = 720 * 1.0f / width;
            float scaleY = 1280 * 1.0f / height;
            if (scaleX > 1.2f || scaleY > 1.2f) {
                if (scaleX > scaleY) {
                    height = 1280;
                    width = (int) (width * scaleY + 0.5f);
                } else {
                    width = 720;
                    height = (int) (height * scaleX + 0.5f);
                }
            }
        }
